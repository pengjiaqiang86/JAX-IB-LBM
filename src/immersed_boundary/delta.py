"""
Regularised delta-function kernels for IB interpolation / spreading.

A D-dimensional delta kernel is the tensor product of 1-D kernels:
    δ_h(x) = φ(x/dx) * φ(y/dy) [* φ(z/dz)]
...
def delta_nd(
    x_euler: jnp.ndarray,    # (D,)  Eulerian grid point physical position
    x_lag:   jnp.ndarray,    # (D,)  Lagrangian marker physical position
    spacing: tuple,           # (D,)  grid spacings (dx, dy[, dz])
    kernel:  DeltaKernel,
) -> float:
    """
    D-dimensional delta weight as tensor product of 1-D kernels.

    Returns  φ(r_0/dx_0) * φ(r_1/dx_1) [* φ(r_2/dx_2)]
    where r_d = x_euler[d] - x_lag[d].
    """
    weight = 1.0
    for d in range(len(spacing)):
        r = (x_euler[d] - x_lag[d]) / spacing[d]
        weight = weight * kernel.phi(r)
    return weight
